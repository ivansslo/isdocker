...
# ─────────────────────────────────────────────────────────────────
#  roc-containers · Calibre-Web (eBook Manager)
...
if [ -n "$1" ]; then
  cmd="$*"
  udocker_run --entrypoint "bash -c" -p "${PORT}:8083" \
    -e CALIBRE_PORT="$PORT" -e TZ="$(get_tz)" \
    -v "$DATA_DIR/config:/config" \
    -v "$DATA_DIR/books:/books" \
    "$CONTAINER_NAME" "$cmd"
else
  udocker_run --entrypoint "bash -c " -p "${PORT}:8083" \
    -e CALIBRE_PORT="$PORT" -e TZ="$(get_tz)" \
    ...
    "$CONTAINER_NAME" '
      ...
      if [[ ! -f /books/metadata.db ]]; then
          curl -sL -o /books/metadata.db \
              "https://github.com/janeczku/calibre-web/raw/refs/heads/master/library/metadata.db"
      fi
