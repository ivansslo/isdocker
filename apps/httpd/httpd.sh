...
if [ -n "$1" ]; then
  cmd="$*"
  udocker_run --entrypoint "bash -c" -p "${PORT}:80" \
    -v "$DATA_DIR/htdocs:/usr/local/apache2/htdocs" \
    "$CONTAINER_NAME" "$cmd"
else
  udocker_run --entrypoint "bash -c" -p "${PORT}:80" \
    -e _PORT="$PORT" \
    ...
    "$CONTAINER_NAME" '
      ...
      _PIDFILE="$(mktemp)"
      start-stop-daemon -mp "$_PIDFILE" -bSa "$(command -v bash)" -- \
          -c "exec $(command -v httpd-foreground) >/var/log/httpd/daemon.log 2>&1" && \
